                            ...
                            <div id="item-selection-container" class="hidden">
                                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                    ...
                                    <div class="lg:col-span-2">
                                        <div id="denah" class="bg-white rounded-xl border border-gray-200 overflow-hidden">
                                            <div class="bg-gray-50 px-4 py-3 border-b border-gray-200">
                                                ...
                                                <div class="flex flex-wrap gap-4 mt-2 text-xs text-gray-600">
                                                    ...
                                                    <div class="flex items-center gap-1">
                                                        <div
                                                            class="w-3 h-3 rounded-full bg-blue-600 border border-blue-200">
                                                        </div>
                                                        <span>Dipilih</span>
                                                    </div>
                                                    <div class="flex items-center gap-1">
...
@section('script')
    <script>
        ...
        function selectDesk(element, index, deskEncoded) {
            ...
            // 1. Hapus class selected dari semua card
            document.querySelectorAll('.desk-card').forEach(card => {
                card.classList.remove('selected');
            });

            // 2. Tambahkan class selected LANGSUNG ke elemen yang diklik
            if (element) {
        ...
        function syncListHeight() {
            ...
            // Ambil elemen header di atas list (supaya kalkulasi akurat)
            const listHeader = listContainer ? listContainer.previousElementSibling : null;

            if (denah && listContainer) {
                // 1. Ambil tinggi total kartu kiri (Denah)
                const denahHeight = denah.offsetHeight;

                // 2. Ambil tinggi header kartu kanan
                // Kita harus kurangi ini, kalau tidak kartu kanan akan lebih panjang dari kiri
                const headerHeight = listHeader ? listHeader.offsetHeight : 0;

                // 3. Hitung tinggi bersih untuk area scroll
                const finalHeight = denahHeight - headerHeight;

                // 4. Terapkan style
                // Kita set 'height' juga agar flex container terisi penuh, 
                // dan 'maxHeight' untuk membatasi scroll.
